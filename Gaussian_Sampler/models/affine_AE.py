...
class Affine_AE_2D(nn.Module):
    def __init__(self,
        ...
        super(Affine_AE_2D, self).__init__(affine_kwargs=affine_kwargs)
        
        self.device = device
        
        for key, value in affine_encoder_kwargs.items():
            setattr(self, key, value)
        self.encoder = affine_encoder(**affine_encoder_kwargs)
        
        affine_kwargs['device'] = device
        for key, value in affine_kwargs.items():
            setattr(self, key, value)
        self.affine_model = affine_model(**affine_kwargs)
        
        
            
            
        self.affine = Affine_Transform(self.device, scale = True, shear = True, rotation = True, translation = True, Symmetric = True, mask_intensity = True)


        self.decoder = nn.Sequential(
            nn.Linear(self.embedding_size, 2*16*16),
            nn.ReLU(),
            nn.Unflatten(1, (2, 16, 16)),
            nn.ConvTranspose2d(2, 4, 3, stride=2, padding=1, output_padding=1),
            nn.ReLU(),
    ...
    def forward(self, x):
        emb_affine = self.encoder(x)
        scaler_shear, rotation, translation, mask_parameter = self.affine(x)
        x = self.decoder(x)
        return x, scaler_shear, rotation, translation, mask_parameter
